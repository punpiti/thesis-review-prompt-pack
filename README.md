...
เป้าหมายคือฝึกให้นิสิตช่วยกันตรวจและให้ feedback ที่มีหลักฐาน ไม่ใช่ให้ AI ปรับปรุงงานแทนนิสิตโดยอัตโนมัติ ชุดนี้ช่วยให้เห็นว่าเอกสารมีปัญหาอะไรที่ควรแก้ก่อน เช่น ปัญหาวิจัยไม่ชัด วัตถุประสงค์ไม่ตรงกับผล literature review ยังไม่สังเคราะห์ reference ไม่พอ baseline ขาด หรือ claim แรงเกินหลักฐาน ไม่ใช่ให้ AI ตัดสินแทนผู้วิจัยหรือแทน reviewer จริง
...
ค่าเริ่มต้นของชุดนี้ตั้งใจให้เด็กใหม่ใช้ได้ก่อน: จัดไฟล์ สร้าง instruction แล้วสั่ง `ประเมิน` เพื่อให้เห็นปัญหาใหญ่และ action ที่ต้องทำ ไม่จำเป็นต้องใช้ prompt ขั้นสูงตั้งแต่รอบแรก

ถ้างานมีความก้าวหน้ามากขึ้นแล้ว เช่น มี draft หลักครบ มี reference/result พอสมควร และมี review notes รอบแรกแล้ว ค่อยใช้ `ประเมินเชิงลึก` เป็น advanced pass ระดับ reviewer/advisor หรือปริญญาเอก prompt นี้แยกไว้ที่ [06_deep_doctoral_review_prompt.md](06_deep_doctoral_review_prompt.md) เพื่อทดสอบ source triangulation, current consensus/debate, counter-argument, claim credibility, data interpretation และ decision brief โดยไม่ทำให้ prompt หลักยาวเกินไป

## Difficulty Levels

| Level | เหมาะกับใคร | ใช้คำสั่ง | ผลลัพธ์หลัก |
|---|---|---|---|
| Beginner | เด็กใหม่ ยังไม่รู้ว่าไฟล์ไหนสำคัญหรือ draft ขาดอะไร | `init workspace จากไฟล์รวม` แล้ว `เตรียม instruction จากงานปัจจุบัน` | file inventory, document map, missing inputs |
| Standard | draft เริ่มมีโครงครบ ต้องการ feedback ก่อนส่งอาจารย์ | `ประเมิน` | full review, reference audit, action checklist, review state |
| Advanced | draft มีหลักฐานมากขึ้น และต้องการทดสอบ argument ให้หนักขึ้น | `ประเมินเชิงลึก` | deep review, counter-arguments, claim audit, decisive tests |

สำหรับนิสิตทั่วไป ให้หยุดที่ `ประเมิน` ก่อน อ่าน review notes แล้วให้เจ้าของงานตัดสินใจแก้ blocking issues เองก่อนใช้ระดับ Advanced

## Student Reviewer Responsibility

ชุดนี้ใช้ AI เป็นผู้ช่วยอ่านและจัด feedback เท่านั้น นิสิตที่ตรวจงานต้องอ่าน review notes, ตรวจหลักฐาน, และอธิบายเหตุผลของ feedback เอง ส่วนเจ้าของงานต้องเป็นคนตัดสินใจปรับแก้ต้นฉบับเอง

หน้าที่ของ AI และเพื่อนผู้ตรวจคือชี้จุดอ่อน แนะนำแนวทาง และบอกหลักฐานที่ควรตรวจเพิ่ม จากนั้นเจ้าของงานต้องกลับไปคิดและทำเอง

อย่าใช้ workflow นี้เป็น auto-rewrite หรือ auto-improvement pipeline ถ้าจะให้ AI ช่วยแก้ข้อความ ต้องทำหลังจากมี review notes แล้วเท่านั้น และต้องทำบน working copy พร้อมตรวจ diff ก่อนรับการแก้ทุกครั้ง

ถ้านิสิตจะใช้ AI ต่อเพื่อช่วยพัฒนา argument, experiment, literature search, หรือ revision plan ให้นิสิตควรฝึกเขียน prompt เองจาก review notes แทนการให้ชุดนี้สั่ง AI ทำแทนทั้งหมด
...
- [01_full_thesis_review_prompt_explained_en.html](01_full_thesis_review_prompt_explained_en.html) - English HTML explanation of the main prompt concept
- [01_full_thesis_review_prompt_explained_en.md](01_full_thesis_review_prompt_explained_en.md) - English Markdown explanation of the main prompt concept
...
- [06_deep_doctoral_review_prompt.md](06_deep_doctoral_review_prompt.md) - prompt เสริมสำหรับตรวจเชิงลึกระดับปริญญาเอกหลังจากมี review notes รอบแรก
...
ให้รัน install command ใน [workspace.html](workspace.html) จากโฟลเดอร์ที่ต้องการสร้างงาน command จะดาวน์โหลด zip และสร้าง project folder จาก `workspace_template/`
...
installer ทำแค่ดาวน์โหลดชุด prompt, แตก zip, และสร้าง workspace จาก template เท่านั้น หลังจากนั้นนิสิตยังต้องวางไฟล์เอง อ่าน review notes เอง และคิด prompt ต่อเองจากปัญหาที่พบ
...
- AI ช่วยอ่านและจัดระบบ feedback ได้ แต่ผู้ตรวจและผู้เขียนต้องรับผิดชอบเหตุผลของ feedback และ claim ทุกข้อเอง
- ชุดนี้ไม่ใช่ระบบปรับปรุงต้นฉบับอัตโนมัติ ให้สร้าง review notes ก่อนเสมอ แล้วให้เจ้าของงานตัดสินใจแก้เอง
- เป้าหมายคือชี้จุดอ่อน แนะนำ และให้เจ้าของงานไปทำเอง ไม่ใช่ให้ AI ทำ revision ให้ครบแทน
- ถ้าต้องใช้ AI ต่อ นิสิตควรฝึกตั้ง prompt จากปัญหาที่พบเอง เช่น ขอ checklist, ขอคำถามตรวจ source, หรือขอทางเลือก experiment ไม่ใช่สั่งให้ rewrite ทั้งงาน
...
ถ้าต้องการเข้าใจว่าทำไม prompt ตรวจหัวข้อเหล่านี้ ให้อ่าน [01_full_thesis_review_prompt_explained_th.html](01_full_thesis_review_prompt_explained_th.html) หรือ English version ที่ [01_full_thesis_review_prompt_explained_en.html](01_full_thesis_review_prompt_explained_en.html)
...
### Round 4: Deep Doctoral Review

ใช้เป็นขั้นสูงเท่านั้น เมื่อมีผลตรวจรอบแรกแล้วและงานมีเนื้อหาพอให้โจมตี argument ได้ เช่น ก่อน proposal defense, defense, manuscript submission หรือ revision ใหญ่:

```text
ประเมินเชิงลึก
```

AI ควรใช้ [06_deep_doctoral_review_prompt.md](06_deep_doctoral_review_prompt.md), สร้าง `04_review_notes/04_deep_review.md`, เพิ่ม action ที่พบใหม่ใน `04_review_notes/03_student_action_checklist.md` และอัปเดต `04_review_notes/99_review_state.md`

ถ้า draft ยังต้นมาก หรือยังไม่มี reference/result พอ ให้ยังไม่ต้องใช้ `ประเมินเชิงลึก` เพราะจะได้รายการ missing input มากกว่าคำแนะนำที่แก้ได้ทันที

### Round 5: Fix and Recheck

ให้ AI สรุป `04_review_notes/03_student_action_checklist.md` เพื่อเป็น feedback จากการตรวจ แล้วผู้เขียนเป็นคนตัดสินใจแก้ blocking issues เองก่อนแก้ภาษา เช่น problem ไม่ชัด objective ไม่ตรง result ไม่มี baseline หรือ citation รองรับ claim ไม่ได้
